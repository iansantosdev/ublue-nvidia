...
[![build-ublue](https://github.com/iansantosdev/ublue-nvidia/actions/workflows/build.yml/badge.svg)](https://github.com/iansantosdev/ublue-nvidia/actions/workflows/build.yml)

The purpose of these images is to provide [community Fedora images](https://github.com/iansantosdev/ublue-main) with Nvidia drivers built-in. This approach can lead to greater reliability as failures can be caught at the build level instead of the client machine. This also allows for individual sets of images for each series of Nvidia drivers, allowing users to remain current with their OS but on an older, known working driver. Performance regression with a recent driver update? Reboot into a known-working driver after one command. That's the goal!
...
- CUDA support
- [Container runtime support](https://github.com/iansantosdev/ublue-nvidia#using-nvidia-gpus-in-containers)
- Secure boot
- [Hardware-accelerated video playback](https://github.com/iansantosdev/ublue-nvidia#video-playback)
- Selinux support
- [Multiple Fedora flavors and releases](https://github.com/iansantosdev/ublue-nvidia#setup)
- Post-install setup with [`just`](https://github.com/iansantosdev/ublue-nvidia/blob/main/justfile)
...
1. Rebase onto the image
   ...
   Note: The image previously titled "nvidia" will not be updated anymore. If you wish to retain the same functionality, switch to "silverblue-nvidia".
    ...
    [Silverblue (GNOME):](https://github.com/iansantosdev/ublue-nvidia/pkgs/container/silverblue-nvidia)
    ```
    rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/silverblue-nvidia:latest
    ```

    [Kinoite (KDE):](https://github.com/iansantosdev/ublue-nvidia/pkgs/container/kinoite-nvidia)
    ```
    rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/kinoite-nvidia:latest
    ```

    [LXQt (unofficial):](https://github.com/iansantosdev/ublue-nvidia/pkgs/container/lxqt-nvidia)
    ```
    rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/lxqt-nvidia:latest
    ```
    [MATE (unofficial):](https://github.com/iansantosdev/ublue-nvidia/pkgs/container/mate-nvidia)
    ```
    rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/mate-nvidia:latest
    ```

    [Vauxite (XFCE, unofficial):](https://github.com/iansantosdev/ublue-nvidia/pkgs/container/vauxite-nvidia)
    ```
    rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/vauxite-nvidia:latest
    ```

    [Sericea (Sway, unofficial as of Fedora 38):](https://github.com/iansantosdev/ublue-nvidia/pkgs/container/sericea-nvidia)

    Note: [The Sway session has some custom configuration options set to improve stability on Nvidia GPUs](https://github.com/iansantosdev/ublue-nvidia/blob/main/files/etc/sway/environment)

    ```
    rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/sericea-nvidia:38
    ```

    [Base (no DE preinstalled):](https://github.com/iansantosdev/ublue-nvidia/pkgs/container/base-nvidia)
    ```
    rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/base-nvidia:latest
    ```
...
## Rolling back and rebasing
   ...
   To rebase onto a specific date, use a date tag:
       ...
       rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/silverblue-nvidia:20230128

   Or to rebase onto a specific release, driver, and date:

       rpm-ostree rebase ostree-unverified-registry:ghcr.io/iansantosdev/silverblue-nvidia:37-525-20230128

   [More options for image tags can be found on the container catalog.](https://github.com/iansantosdev/ublue-nvidia/pkgs/container/silverblue-nvidia/versions)
...
It is *strongly encouraged* for you to subscribe to [the Nvidia driver announcements](https://github.com/orgs/iansantosdev/discussions/categories/nvidia-driver-announcements?discussions_q=is%3Aopen+category%3A%22Nvidia+Driver+Announcements%22) section of the forums to keep up with the latest changes and news.
